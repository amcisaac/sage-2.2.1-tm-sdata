...
from openff.qcsubmit.results.filters import SinglepointRecordFilter, T
...
def download_td_data(
    ...
    filtered_for_charge = new_dataset.filter(ChargeCheckFilter(nprocs=n_processes))
...
def download_opt_data(
    ...
    filtered_for_charge = new_dataset.filter(ChargeCheckFilter(nprocs=n_processes))
    filtered_for_charge = filtered_for_charge.filter(ConformerRMSDFilter(max_conformers=max_opt_conformers))
    if verbose:
        print(f"Number of entries after charge check: {filtered_for_charge.n_results}")
